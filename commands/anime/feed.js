...
module.exports = {
	...
	run: async (client, message, args) => {
		if (!args.join(" ")) return message.reply("You need to specify a person to feed");
		...
		const img = await neko.sfw.feed();
		const embed = new MessageEmbed();
		embed.setColor("Black");
		embed.setAuthor({name: message.author.username, iconURL:message.author.avatarURL()});
		embed.setDescription(`**${member.user.username}** has been fed by **${message.author.username}**`);
